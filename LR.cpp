...
void slave_main(uint32_t tid)
{
	...
	for (int itr = 0; itr < ITER_NUM; itr++)
	{
		...
		//std::cout << "grad=" << local_grad_arr[0][0] <<" local_param=" <<local_param[0] << std::endl;
		loss[0] = 0;
...
void master()
{
	...
	float* dummy_param= new float[g_param_len];
	...
	for (int itr = 0; itr < ITER_NUM; itr++)
	{
		memcpy(dummy_param,g_param,sizeof(float)*g_param_len);
		auto t2 = std::chrono::system_clock::now();
		MPI_Reduce(&dummy , &outloss ,1, MPI_FLOAT , MPI_SUM ,0, MPI_COMM_WORLD );
		MPI_Allreduce(dummy_param , g_param , g_param_len, MPI_FLOAT , MPI_SUM , MPI_COMM_WORLD );
		std::cout << "Iter"<<itr<<" took "<<
			std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - t2).count()
			<<" milliseconds, loss = " <<outloss<<std::endl;
	}
